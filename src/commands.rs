...
pub struct Command {
    ...
    exec: fn(files: &mut DataFiles)
}

impl Command {

    fn new(name: &str, aliases: Option<&'static [&str]>, exec: fn(files: &mut DataFiles)) -> Self {
        Command {
...
pub fn handle_command(command: String, files: &mut DataFiles) {
...
pub fn get_commands() -> [Command; 8] {
    return [
        ...
        Command::new("reload", None, reload_command)
    ];
}